...
const MessageList = ({ messages = [], request, request_id }) => {
  ...
  const listOfMessages = messages.map((message) => {
    ...
    return (
      ...
      <div className={clsx({"self-end bg-blue-600 text-white border border-indigo-600 rounded-full shadow-sm py-1 px-4 text-md": currentUser.id === message.sender_id, "self-start bg-slate-200 text-black border border-grey-600 rounded-full shadow-sm py-1 px-4 text-md": currentUser.id !== message.sender_id })}>
          <MessageListItem
            key={message.id}
            id={message.id}
            body={message.body}
